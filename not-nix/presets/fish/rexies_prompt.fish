#### CREDITS ####
# Written by Zaphkiel/Rexiel Scarlet
# https://radicle.defelo.de/nodes/radicle.defelo.de/rad:z3qZMsGeZUYKgfpX9NdDYKEaZr4oq/tree/dots/fish/config.fish
#################
...
function __rexies_prompt_jj_async --on-event fish_prompt
    if ! set -q __rexies_prompt_jj_color
        set -g __rexies_prompt_jj_color $__rexies_color_tertiary
...
function __rexies_prompt_direnv
    if test "$IN_NIX_SHELL" = impure
        echo "$(set_color $__rexies_color_primary)!"
    else
        echo "$(set_color $__rexies_color_primary)@"
    end
end
...
function __rexies_prompt_host
    echo "$(set_color $__rexies_color_primary)$hostname"
end

function __rexies_prompt_pwd
    echo "$(set_color $__rexies_color_secondary)/$(basename $PWD)"
end

function __rexies_prompt_head
    echo "$(set_color $__rexies_color_secondary)>$(set_color --reset)"
end
...
function __rexies_render_prompt --on-signal USR1
    set __rexies_async_counter (math $__rexies_async_counter - 1)
    if test "$__rexies_async_counter" -ne 0
        return
    end
