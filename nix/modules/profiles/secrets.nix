# THIS IS AI GENERATED BE WARNED
{
  flake.nixosModules.secret-assertions =
    ...
    let
      ...
      hostAlias =
        if config.security.nix-secrets.hostRecipientAlias != null then
          config.security.nix-secrets.hostRecipientAlias
        else
          hostName;

      hostKey = config.security.nix-secrets.recipientAliases.${hostAlias} or null;

      keysFor = k: if builtins.isList k then k else [ k ];

      undecryptable = lib.filterAttrs (
